...
class publicar extends coneccion{
	...
	public function controles($titulo,$nomBoton=false,$target=false){
		echo '<div class="row">';
	echo "<div class='col-md-10'>";
	echo "<div style='margin-bottom:20px;'><h3>".$titulo."</h3></div>";
	echo "</div>";
	if($nomBoton!=false){
	echo "<div class='col-md-2'>";
	echo "<a href='".$target."' style='width:100%;' class='btn btn-primary btn-sm' role='button'><i class='fas fa-user-plus'></i>&nbsp;".$nomBoton."</a>";
	echo "</div>";
	}
	echo "</div>";
	}
 ...
 <input type="button"  style="padding-left:10px; padding-right:10px;" id="search" value="Buscar"  />(ej:republica 400)';
	...
	public function devolverPrecioEn($id){
		$arrSel12=array("1"=>"Pesos","2"=>"U.F.");
		return($arrSel12[$id]);
	}
	public function modificarPropiedad($id=false){
		if(isset($_GET["idq"])){
			$id=htmlentities($_GET["idq"]);
		}
		$this->link=$this->conectar();	
		
		$cadena="select* from mm_propiedad1 where idProp='".$id."'";
	   ...
	   if(isset($_GET["msg"])){
		   ...
		   if($msg==1){
                echo '<div class="alert alert-primary" role="alert">
                Cambios fueron guardados con exito !!!
              </div>';
		   }
	   }     
	   if($_SESSION["auth"]["tipo"]=="admin"){

		echo "<div>Codigo de la Propiedad</div>";
		echo "<div  style='margin-bottom:0px;'><input type='text' value='".$row["codigoProp"]."' placeholder='Codigo' name='codProp' id='codProp' class='form-control from-control-sm'/></div>";
 


	   echo "<div>Propietario</div>";
	   echo "<div>";
	   echo "<select class='form-select from-select-mb' id='propietario' name='propietario'>";

	   echo "<option value='".$row["idPropietario"]."' selected>".$this->devolverPropietario($row["idPropietario"])."</option>";
	   $sql="select* from mm_propietarios order by idPropietario desc";
	   $q=mysqli_query($this->link,$sql);
	   while($row1=mysqli_fetch_array($q)){
		echo "<option value='".$row1["idPropietario"]."'>".$row1["nombre"]."&nbsp;".$row1["apellido"]."/".$row1["rut"]."</option>";
	   }
	   echo "</select>";
	   echo "</div>";
	}else{
		echo "<input type='hidden' name='propietario' id='propietario' value='".$_SESSION["auth"]["idReg"]."'/>";
	}
	   echo "<div>Titulo</div>";
	   echo "<div><input type='text' placeholder='Titulo de la propiedad' name='titulo' id='titulo' value='".$row["titulo"]."' class='form-control from-control-sm'/></div>";
	   $arrSel=array(1=>"Venta",2=>"Arriendo",3=>"Arriendo por dias",4=>"Arriendo marzo a diciembre",5=>"Arriendo año corrido",6=>"División Usados");
	   ...
	   echo "<div>";
	   echo "<select style='width:100%;' name='operacion' id='operacion' class='form-control form-control-mb'>";
	   echo "<option value='".$row["operacion"]."' selected='selected'>".$this->devolverOperacion($row["operacion"])."</option>";
	   foreach($arrSel as $c=>$v){
		   echo "<option value='".$c."'>".$v."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div>";
	   echo "<select name='estadoProp' id='estadoProp' class='form-control form-control-mb'>";
	   echo "<option value='".$row["estadoProp"]."' selected='selected'>".$this->devolverEstadoProp($row["estadoProp"])."</option>";
	   foreach($arrSel1 as $c1=>$v1){
		   echo "<option value='".$c1."'>".$v1."</option>";
	   }
	   echo "</select>";
...
*/
	   ...
	   echo "<div style='margin-top:10px;'>Región</div>";
	   echo '<select  class="form-control form-control-mb" name="region" id="region"  style="margin-top:5px;margin-bottom:5px;" aria-label=".form-select-lg example">';
	   echo '<option value="'.$row["idRegion"].'" selected="selected">'.$this->devolverRegion($row["idRegion"]).'</option>';
		  $sql="select* from mm_region order by idRegion asc";
		  $q=mysqli_query($this->link,$sql);
		  while($r=mysqli_fetch_array($q)){
			  echo '<option value="'.$r["idRegion"].'">'.utf8_encode($r["nombre"]).'</option>';
	   ...
	   echo '	<select name="ciudad"  style="margin-top:5px;margin-bottom:5px;" id="ciudad"
	   class="form-control form-control-mb" aria-label=".form-select-lg example">
	  <option value="'.$row["idCiudad"].'" selected="selected">'.$this->devolverCiudad($row["idCiudad"]).'</option>
	   ...
	   echo '<select   style="margin-top:5px;margin-bottom:5px;" name="comuna" id="comuna"
	   class="form-control form-control-mb" aria-label=".form-select-lg example">
	  <option value="'.$row["idComuna"].'" selected="selected">'.$this->devolverComuna($row["idComuna"]).'</option>	   
	  </select>';
	   echo "</div>";
	   ...
	   echo "<div>";
	   echo "<select name='precioUf' id='precioUf' placeholder='Precio en' class='form-control form-control-mb'>";
	   echo "<option value='".$row["precioUf"]."' selected='selected'>".$this->devolverPrecioEn($row["precioUf"])."</option>";
	   foreach($arrSel12 as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div>";
	   echo "<select name='bodega' id='bodega' class='form-control form-control-mb'>";
	   echo "<option value='".$arrSel1[$row["bodega"]]."' selected='selected'>".$arrSel1[$row["bodega"]]."</option>";
	   foreach($arrSel1 as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div>";
	   echo "<select name='logia' id='logia' class='form-control form-control-mb'>";
	   echo "<option value='".$arrSel1[$row["logia"]]."' selected='selected'>".$arrSel1[$row["logia"]]."</option>";
	   foreach($arrSel1 as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div>";
	   echo "<select name='numEstacionamientos' id='numEstacionamientos' class='form-control form-control-mb'>";
	   echo "<option value='".$row["estacionamiento"]."' selected='selected'>".$arrSel12[$row["estacionamiento"]]."</option>";
	   foreach($arrSel12 as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div>";
	   echo "<select name='cocina' id='cocina' class='form-control form-control-mb'>";
	   echo "<option value='".$row["cocina"]."' selected='selected'>".$arrSel12[$row["cocina"]]."</option>";
	   foreach($arrSel12 as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div>";
	   echo "<select name='tipoCocina' id='tipoCocina' class='form-control form-control-mb'>";
	   echo "<option value='".$row["tipoCocina"]."' selected='selected'>".$arrSel122[$row["tipoCocina"]]."</option>";
	   foreach($arrSel122 as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div style='margin-top:10px;'>Conserjería</div>";
	   echo "<select name='conser' id='conser' class='form-control form-control-mb'>";
	   echo "<option value='".$row["concer"]."' selected='selected'>".$arrSel122a[$row["conser"]]."</option>";
	   foreach($arrSel122a as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div style='margin-top:10px;'>Quincho</div>";
	   echo "<select name='quincho' id='quincho' class='form-control form-control-mb'>";
	   echo "<option value='".$row["quincho"]."' selected='selected'>".$arrSel122a[$row["quincho"]]."</option>";
	   foreach($arrSel122a as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
	   ...
	   echo "<div style='margin-top:10px;'>Áreas comunes</div>";
	   echo "<select name='areasComunes' id='areasComunes' class='form-control form-control-mb'>";
	   echo "<option value='".$row["areasComunes"]."' selected='selected'>".$arrSel122a[$row["areasComunes"]]."</option>";
	   foreach($arrSel122a as $c2=>$v2){
		   echo "<option value='".$c2."'>".$v2."</option>";
	   }
	   echo "</select>";
...
*/
	   echo "<div><input type='hidden' name='action' id='action' value='true' class='form-control form-control-mb'/></div>";
	   echo "<div style='margin-top:10px;'><button role='button' id='s1' name='s1' class='btn btn-primary btn-sm'>Guardar Cambios</button></div>";
	   echo "<div>&nbsp;</div>";
	   echo "<div>&nbsp;</div>";
	   echo "<div>&nbsp;</div>";
	   echo "<div>&nbsp;</div>";
		...
		if(isset($_POST["action"])){
            ...
            if(mysqli_query($this->link, $sql)) {
				/*
                    ...
                    foreach($arch as $c=>$v){
                        $sql="insert into mm_cape_fotos1 (idProp,ruta,portada) values ('".$idRegistroIng."','".$v."','1')";            
			...
			if($_SESSION["auth"]["tipo"]=="admin"){
				echo '<script>
				document.location="panel.php?op=1&mq=editar&idq='.$id.'&msg=1";				
				</script>';
				header("location:panel.php?op=1&mq=editar&idq=".$id."&msg=1");			 
				exit;		 	
			}else{
				header("location:panelPropietario.php?op=1&mq=editar&idq=".$id."&msg=1");			 
				exit;		 	
			}
    ...
    public function ingresarPropiedad(){
	   ...
	   if(isset($_GET["msg"])){
		   ...
		   if($msg==1){
			...
			<strong>¡Éxito!</strong> Propiedad Ingresada con exito.
	   ...
	   if($_SESSION["auth"]["tipo"]=="admin"){
		echo "<div>Codigo de la Propiedad</div>";
		echo "<div  style='margin-bottom:0px;'><input type='text' placeholder='Codigo' name='codProp' id='codProp' class='form-control from-control-sm'/></div>";
 
		echo "<div>Propietario</div>";	
	   echo "<div>";
	   echo "<select class='form-select from-select-mb' id='propietario' name='propietario'>";
	   echo "<option value='0' selected>Seleccione propietario</option>";
	   $sql="select* from mm_propietarios order by idPropietario desc";
	   $q=mysqli_query($this->link,$sql);
	   while($row=mysqli_fetch_array($q)){
		echo "<option value='".$row["idPropietario"]."'>".$row["nombre"]."&nbsp;".$row["apellido"]."/".$row["rut"]."</option>";
	   }
	   echo "</select>";
	   echo "</div>";
		}else{
			echo "<div><input type='hidden' placeholder='Propietario' name='propietario' id='propietario' value='".$_SESSION["auth"]["idReg"]."' class='form-control from-control-sm'/></div>";	
		}
...
*/
	   ...
	   echo "<div>&nbsp;</div>";
		...
		if(isset($_POST["action"])){
			...
			if($_SESSION["auth"]["tipo"]=="admin"){				
				echo '<script>
				document.location="panel.php?op=5&msg=1";
				</script>';	 
				exit;		 	
			}else{
				echo '<script>
				document.location="panelPropietario.php?op=28&msg=1";
				</script>';	 				
				exit;		 	
			}
	...
	public function tablaPropiedadesUser($idPropi){ 
        ...
        if(isset($_GET["mq"])){$op=htmlentities($_GET["mq"]);}
		  if($op=="agregar") {
			...
			//$this->controles("Modificar Propiedad","Agregar Propiedad","panel.php?op=5");
            $id=htmlentities($_GET["idq"]);   
			...
			$this->controles("Tus Propiedades","Agregar Propiedad","panelPropietario.php?op=5");
        if($tipo=="admin"){
			$index="panel.php?op=1";
		}else{
			$index="panelPropietario.php?op=27";
		}
		...
		if(isset($_POST["action"])){
			...
			if($col==1){
				// operacion
				if($tipo=="admin"){
					$sql="SELECT * FROM `mm_propiedad1` where operacion='".$buscador."' order by idProp desc";
				}else{
					$sql="SELECT * FROM `mm_propiedad1` where operacion='".$buscador."' and idPropietario='".$idPropi."' order by idProp desc";
				}
			}else if($col==2){
				// tipo Propiedad
				if($tipo=="admin"){
					$sql="SELECT * FROM `mm_propiedad1` where tipoProp='".$buscador."'  order by idProp desc";
				}else{
					$sql="SELECT * FROM `mm_propiedad1` where tipoProp='".$buscador."' and idPropietario='".$idPropi."' order by idProp desc";
				}
				...
				if($tipo=="admin"){
					...
					$sql="SELECT * FROM `mm_propiedad1` where idPropietario='".$idPropi."' and idPropietario='".$idPropi."' order by idProp desc";
				}
			}
		}else{
			if($tipo=="admin"){
		...
		<select name='columna' id='columna' class='form-select form-select-sm'>
				...
				<option value='2'>Tipo de Propiedad</option>				
		</select>
		</div>
		<div class='col-md-3'>
 	...
 	public function tablaPropiedades(){ 
        ...
        if($tipo=="admin"){
			$index="panel.php?op=1";
			$this->controles("Propiedades","Agregar Propiedad","panel.php?op=5");
		}else{
			$this->controles("Propiedades","Agregar Propiedad","panelPropietario.php?op=5");
        ...
        if(isset($_GET["mq"])){$op=htmlentities($_GET["mq"]);}
		  if($op=="agregar") {
			...
			$sql="delete from mm_propiedad1 where idProp='".$id."'";     
			mysqli_query($this->link,$sql);
        ...
        $campos=array("codigoProp"=>"codigoProp",
        ...
        if(isset($_GET["msg"])){
            echo '<div class="alert alert-primary" style="padding:5px;font-size:14px;" role="alert" id="alert-message">
				<strong>¡Éxito!</strong> Propiedad borrada con éxito.
