...
class sql extends coneccion{
	...
	public function sqlIngresarVivienda($d){
		...
		$sql="insert into mm_propiedad1 (codigoProp,idCorredora,idPropietario, idRegion,idCiudad,idComuna,titulo ,operacion ,estadoProp ,direccionProp,tipoProp ,ciudad ,codigo ,precio ,m2Construido ,mt2Totales,piscina   ,dormitorios ,banos ,cocina   ,tipoCocina,comedor ,destacar,estacionamiento ,bodega ,logia,fecha ,descripcion,PrecioUf,cordenadas,conser,quincho,areasComunes) ";
		...
		$sql.=" values ( 
			...
			'".$d["comuna"]."',
					   ...
					   '".$fecha."',
					   '".$d["des"]."','".$d["precioUf"]."','".$cor."', '".$d["conser"]."', '".$d["quincho"]."', '".$d["areasComunes"]."')";
	...
	public function sqlModificarVivienda($d,$id,$c){
	 $m2Totales=str_replace(".","",$d["m2Totales"]);
		...
		$sql="update mm_propiedad1 set  codigoProp='".$d["codProp"]."', 
									idPropietario ='".$d["propietario"]."',
									idRegion ='".$d["region"]."',
									idCiudad ='".$d["ciudad"]."',
									idComuna ='".$d["comuna"]."',
								    titulo ='".$d["titulo"]."',
